...
def InitCreature (nameCreature):
    '''Создаем животное с известным именем и случайными данными'''
    name = input('Дайте имя созданию ({}) или нажмите ввод чтобы оставить имя по умолчанию: '.format(nameCreature))
...
def Attacked(whoAttacked):
    '''Атакуем. Параметр whoAttacked = True: first атакует second'''

    global first
    global second

    def Attack(agressor, victim):
        def GetDamage(player, damage):
            return int(damage / player['armour'])

        return victim['health'] - GetDamage(victim,agressor['damage'])
